...
-(void)initDataArr{
    NSArray * arr1 =@[@"清除缓存",@"界面跳转动画效果"];
    NSArray * arr2 =@[@"关于我们",@"提交错误与建议",@"当前版本:1.2"];
...
-(CGFloat)getCaheSize{
    ...
    NSString * myCachePath = [NSHomeDirectory() stringByAppendingString:@"First·简/Caches"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section == 0) {
        ...
        switch (indexPath.row) {
            case 0://清除缓存
            {
                ...
                [sheet addAction:[UIAlertAction actionWithTitle:@"删除" style:UIAlertActionStyleDestructive handler:^(UIAlertAction *action) {
                    ...
                    //2.删除自己缓存
                    NSString *myCachePath = [NSHomeDirectory() stringByAppendingPathComponent:@"First·简/Caches"];
        ...
        switch (indexPath.row) {
            ...
            case 1:
            {
                ErrorAndAdvice * error = [[ErrorAndAdvice alloc]init];
